...
Compliance as Code
------------------
When operating LZ Watch Dog, it is highly recommended to treat the policy
...
This configuration will install LZ Watch Dog and validate the policy.yml file
...
To run LZ Watch Dog against your account, you will need an IAM role with appropriate
...
LZ Watch Dog is to start with running LZ Watch Dog against a single account
on a virtual machine.

To start, create a virtual machine on your cloud provider of choice.
It's recommended to execute LZ Watch Dog in the same cloud provider
...
Once you have LZ Watch Dog installed, download your policies that you created
...
Monitoring LZ Watch Dog
--------------------------

LZ Watch Dog ships with the ability to emit metrics on policy execution and transport
