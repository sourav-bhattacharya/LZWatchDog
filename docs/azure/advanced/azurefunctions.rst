...
If specified resources already exist in the subscription (discoverable by resource group name and resource name), LZ Watch Dog won't make any changes (location, sku)
...
If you provide resource ids, LZ Watch Dog verifies that resource exists before function app provisioning. It returns an error if resource is missing.
...
when any event is triggered in the subscription. LZ Watch Dog filters to the events you passed to your policy and
...
When Management Groups option is used with periodic mode, LZ Watch Dog deploys a single Azure Function App with multiple Azure Functions following single subscription per function rule.

Event triggered functions
=========================

When Management Groups option is used with event mode, LZ Watch Dog deploys single Azure Function. It creates Event Grid subscription for each Subscription in Management Group delivering events to a single Azure Storage Queue.
...
Service Principal used to provision Azure Functions required to have permissions to access Management Groups. If SP doesn't have `MG Reader` permissions in any child subscription these subscriptions won't be a part of LZ Watch Dog Azure Function deployment process.
